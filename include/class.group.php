...
class Group {
    ...
    function save($id,$vars,&$errors) {
        ...
        if(!$vars['name']) {
            $errors['name']=_('Group name required');
        }elseif(strlen($vars['name'])<3) {
            $errors['name']=_('Group name must be at least 3 chars.');
        }elseif(($gid=Group::getIdByName($vars['name'])) && $gid!=$id){
            $errors['name']=_('Group name already exists');
        }
        ...
        if($id) {
            ...
            $errors['err']=_('Unable to update group. Internal error occurred.');
