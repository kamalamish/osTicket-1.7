...
$info=array();
$info['subj']=_('osTicket test email');
...
if($_POST){
    ...
    if(!$errors && $email){
        if($email->send($_POST['email'],$_POST['subj'],$_POST['message']))
            $msg=_('Test email sent successfully to').' '.Format::htmlchars($_POST['email']);
        else
...
<form action="emailtest.php" method="post" id="save">
 ...
 <h2><?= _('Test Outgoing Email')?></h2>
 <table class="form_table" width="940" border="0" cellspacing="0" cellpadding="2">
    <thead>
        <tr>
            <th colspan="2">
                <em><?= _('Emails delivery depends on your server settings (php.ini) and/or email SMTP configuration.')?></em>
            </th>
        </tr>
    </thead>
    <tbody>
        <tr>
            ...
            <td>
                <select name="email_id">
                    <option value="0">&mdash; <?= _('Select FROM Email')?> &mdash;</option>
                    <?php
        ...
        <tr>
            <td colspan=2>
                <em><strong><?= _('Message')?></strong>: <?= _('email message to send.')?></em>&nbsp;<span class="error">*&nbsp;<?php echo $errors['message']; ?></span><br>
